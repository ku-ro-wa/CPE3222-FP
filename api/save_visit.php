...
$action         = $_POST['action']              ?? '';
$id_number      = trim($_POST['id_number']      ?? ($_POST['usc_id_display'] ?? ($_POST['usc_id'] ?? '')));
...
$phone_number   = trim($_POST['phone_number']   ?? ($_POST['contact_number'] ?? ''));
...
function record_visit(PDO $pdo, int $user_id, string $action): void {
    ...
    else if (in_array($action, ['out'])) {
        $stmt = $pdo->prepare(
            'UPDATE check_ins SET check_out_time = NOW() WHERE visitor_id = ? AND check_out_time IS NULL ORDER BY check_in_time DESC LIMIT 1'
        );
        $stmt->execute([$user_id]);
    }
}
 ...
 */
function find_user(PDO $pdo, bool $is_guest, string $id_number, string $first_name, string $last_name): array|false {
    if ($is_guest) {
        $stmt = $pdo->prepare(
...
try {
    // ── REGISTER & SIGN IN ────────────────────────────────────────────────────
    if ($action === 'register') {
        foreach ([$first_name, $last_name, $barangay, $city, $province, $phone_number, $email] as $value) {
            if ($value === '') {
        ...
        $existing = find_user($pdo, $is_guest, $id_number, $first_name, $last_name);
    ...
    // ── SIGN IN / SIGN OUT ────────────────────────────────────────────────────
    $user = find_user($pdo, $is_guest, $id_number, $first_name, $last_name);
